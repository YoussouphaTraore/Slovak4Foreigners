...
export function TopicRacePage() {
  ...
  const topic = topicId ? stage1Topics.find((t) => t.id === topicId) : null;
  const topicLessonIds = topic?.lessonIds ?? [];
  const lessonPool = lessons.filter((l) => topicLessonIds.includes(l.id) && !l.coming_soon);

  const attemptsLeft = store.getTopicRaceAttemptsLeft();
  ...
  const startRace = useCallback(() => {
    if (store.getTopicRaceAttemptsLeft() <= 0) { setPhase('blocked'); return; }
  ...
  useEffect(() => {
    if (phase !== 'finished' || !topicId) return;
  ...
  const bestScore = topicId ? store.getTopicBestScore(topicId) : 0;
  const backTarget = topicId ? `/topic/${topicId}` : '/';
  ...
  if (phase === 'idle') {
    return (
      <div className="min-h-screen bg-[#E8F4DC] flex flex-col items-center justify-center px-6 text-center">
        ...
        <p className="text-base font-semibold text-brand-green mb-2">{topic?.icon} {topic?.title}</p>
        <p className="text-gray-500 mb-2 text-base max-w-xs leading-relaxed">
        ...
        <p className="text-sm text-amber-700 font-semibold mb-2 max-w-xs">
          Reach Turbo Snail (10+ correct &amp; 50%+ accuracy) to unlock the next topic!
        </p>
        <p className="text-sm text-amber-600 font-semibold mb-1">
          +{BONUS_SECONDS}s bonus for every correct answer!
        </p>
        <p className={`text-sm font-bold mb-2 ${attemptsLeft === 1 ? 'text-orange-500' : 'text-gray-400'}`}>
  ...
  if (phase === 'finished') {
    ...
    const topicPassed = raceResult?.isTurboSnail;
        ...
        {topicPassed ? (
          <div className="bg-green-50 border-2 border-green-300 rounded-2xl px-6 py-3 mb-3 max-w-xs">
            <p className="text-sm font-bold text-green-700">
              🎉 {topic?.title} passed! The next topic is now unlocked.
            </p>
          </div>
        ) : (
          <div className="bg-gray-100 border-2 border-gray-300 rounded-2xl px-6 py-3 mb-3 max-w-xs">
            <p className="text-sm font-bold text-gray-600">🔒 Next topic still locked</p>
